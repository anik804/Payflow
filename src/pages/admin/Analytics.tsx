...
export default function Analytics() {
  const { data: transactionStats, isLoading: isTransactionLoading } =
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card>
          ...
          <CardContent className="text-3xl font-semibold">
            {transactionStats?.totalTransaction}
          </CardContent>
        </Card>
            ...
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie
                  ...
                  label
                >
                  {transactionStats?.transactionByTypes?.map(
                    (_: any, index: number) => (
                      <Cell
                        key={`cell-${index}`}
