...
export default function UserManagement() {
  ...
  const { data: allUserData, isLoading: allUserLoading } = useGetAllUserQuery({
    role: "USER",
    page: currentPage,
    limit,
  });

  const [updateUser] = useUpdateWalletMutation();

  const handleUserUpdate = async (walletId: string, isBlocked: boolean) => {
    const toastId = toast.loading(isBlocked ? "Unblocking..." : "Blocking...");
    try {
      const payload = { isBlocked: !isBlocked };

      const res = await updateUser({ walletId, payload }).unwrap();
      if (res.success) {
        toast.success(
          isBlocked
    ...
    <div className="w-full max-w-7xl mx-auto px-5">
      ...
      <div className="border border-muted rounded-md">
        <Table>
          ...
          <TableBody>
            {allUserData?.data?.map(
              (item: {
                name: string;
                _id: string;
                wallet: { isBlocked: boolean; _id: string };
              }) => (
                <TableRow>
                  <TableCell className="font-medium w-full">
                    <Link to={`/admin/agent-management/${item._id}`}>
                      {item.name}
                    </Link>
                  </TableCell>
                  ...
                  {item?.wallet?.isBlocked === true && (
                    <TableCell>
                      <Button
                        onClick={() =>
                          handleUserUpdate(
                            item.wallet._id,
                            item.wallet.isBlocked
                          )
                        }
                        variant="outline"
                      >
